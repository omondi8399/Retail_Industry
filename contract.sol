...
contract Retail {
    ...
    function addProduct(bytes32 name, uint256 price, uint256 stock) public {
        require(msg.sender == owner, "Only the owner can add products");
        products[name] = Product(name, price, stock);
    }

    function updateProductPrice(bytes32 name, uint256 price) public {
        require(msg.sender ==owner, "Only the owner can update product prices");
        require(products[name].price > 0, "Product does not exist.");
        products[name].price = price;
    }

    function updateProductStock(bytes32 name, uint256 stock) public {
    ...
    function purchase(bytes32 name, uint256 quantity) public payable {
    ...
    function getProduct(bytes32 name) public view returns (bytes32, uint256, uint256) {
